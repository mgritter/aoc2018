...
input = [r|position=< 20247,  40241> velocity=<-2, -4>
...
testInput = [r|position=< 9,  1> velocity=< 0,  2>
...
parse :: String -> Star
parse x =
  let tok = splitOneOf "<,>" x in
    Star (read (tok !! 1), read (tok !! 2)) (read (tok !! 4), read (tok !! 5))

character :: Position -> [Position] -> Char
character p ps =  
  if p `elem` ps then '*' else ' '

-- FIXME: horribly inefficient lookup, but it probably doesn't matter
plotStars :: [Position] -> String
plotStars ps =
  let minx = (minimum (map fst ps))
      miny = (minimum (map snd ps))
      maxx = (maximum (map fst ps))
      maxy = (maximum (map snd ps)) in
    intercalate "\n"
      [ [ character (x,y) ps | x <- [minx..maxx] ]
        | y <- [miny..maxy]]

positionAt :: Int -> Star -> Position
positionAt t (Star (px,py) (vx,vy)) = (px + vx*t, py + vy*t)

-- Looking for -10 <= (y1 + t*dy1 - y2 - t*dy2 <= 10
--   -10 <= (y1-y2) + t*(dy1-dy2) <= 10
--   ( -10 - (y1-y2)  ) / (dy1-dy2) <= t <= (10 + (y1-y2) ) / (dy1-dy2)
-- or the reverse if dy1-dy2 is negative
nearIntercept :: Star -> Star -> Maybe (Int,Int)
nearIntercept (Star (_,y1) (_,dy1)) (Star (_,y2) (_,dy2))
  | dy1 == dy2 = Nothing
  | otherwise = let a = (-10 - (y1-y2)) `div` (dy1-dy2)
                    b = (10 - (y1-y2)) `div` (dy1-dy2) in
                  Just (min a b, max a b)

findIntercept :: [Star] -> (Int,Int)
findIntercept (s1:s2:ss) = case (nearIntercept s1 s2) of
  Nothing -> findIntercept (s1:ss)
  Just (a,b) -> (a,b)

inputStars = map parse (lines input)
testStars = map parse (lines testInput)

plotTimestep :: Int -> [Star] -> String
plotTimestep t ss =
  "Time " ++ (show t ) ++ "\n" ++
  ( plotStars (map (positionAt t) ss) ) ++
  "\n\n"

plotIntercept :: [Star] -> String
plotIntercept ss =
  let range = findIntercept ss in
    concat [ plotTimestep t ss | t <- [(fst range)..(snd range)] ]

main :: IO ()
main = 
  putStrLn (show (findIntercept testStars)) >>
  putStrLn (plotIntercept testStars ) >>
  putStrLn (show (findIntercept inputStars)) >>
  putStrLn (plotIntercept inputStars )
